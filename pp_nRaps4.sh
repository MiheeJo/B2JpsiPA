...
anaBct=2 #0: do b-fit(not-analytic fit for b-lifetime), 1: do b-fit(analytic fit for b-lifetime), 2: do NOT b-fit
...
rapbins=(-2.4--0.47 -2.4--1.47 -1.47--0.47 -0.47-1.47 -0.47-0.53 0.53-1.47 1.47-2.4 -1.47-0.53)
...
if [ "$ispbpb" == "1" ]; then
  ...
  ctauErrF=$(echo $prefix | awk 'BEGIN{FS="_"}; {print $1}')
  if [ "$ctauErrF" == "singleMuEtaAll" -o "$ctauErrF" == "singleMuEtaAllMu4GeV" -o "$ctauErrF" == "singleMuEtaBarrel" -o "$ctauErrF" == "mu4GeV" ]; then
    ctauErrFile=/afs/cern.ch/work/m/miheejo/private/cms442_Jpsi/src/pA/ctauErrCheck/pp5.4pb/ctauErrRange_$ctauErrF\_bit1.txt;
  else
    ctauErrFile=/afs/cern.ch/work/m/miheejo/private/cms442_Jpsi/src/pA/ctauErrCheck/pp5.4pb/ctauErrRange_default_bit1.txt;
  fi
else
  ctauErrF=$(echo $prefix | awk 'BEGIN{FS="_"}; {print $2}')
  if [ "$ctauErrF" == "bit1" -o "$ctauErrF" == "cowboy" -o "$ctauErrF" == "sailor" ]; then
...
#      printf "source /afs/cern.ch/sw/lcg/external/gcc/4.3.2/x86_64-slc5/setup.csh; source /afs/cern.ch/user/m/miheejo/thisroot.csh\n" >> $work.csh
      ...
      printf "cd -\n" >> $scripts/$work.csh
      printf "cp %s/%s %s/fit2DData.h %s/fit2DData_pbpb.cpp .\n" $(pwd) Scripts/$work.csh $(pwd) $(pwd) >> $scripts/$work.csh
