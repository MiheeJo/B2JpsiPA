...
if [ "$ispbpb" == "1" ]; then
  ...
  if [ "$ctauErrF" == "singleMuEtaAllMu4GeV" -o "$ctauErrF" == "singleMuEtaBarrel" -o "$ctauErrF" == "mu4GeV" ]; then
    ctauErrFile=/afs/cern.ch/work/m/miheejo/private/cms442_Jpsi/src/pA/ctauErrCheck/pPb12.55nb/ctauErrRange_default_bit1_$ctauErrF.txt;
  else
    ctauErrFile=/afs/cern.ch/work/m/miheejo/private/cms442_Jpsi/src/pA/ctauErrCheck/pPb12.55nb/ctauErrRange_default_bit1_singleMuEtaAll.txt;
  fi
else
  ctauErrF=$(echo $prefix | awk 'BEGIN{FS="_"}; {print $2}')
  if [ "$ctauErrF" == "bit1" -o "$ctauErrF" == "cowboy" -o "$ctauErrF" == "sailor" ]; then
